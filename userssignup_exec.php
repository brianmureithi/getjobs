...
$target="assets/profile/".basename($_FILES['profile']['name']);
	...
	//see if that member is an identical match to another in the system
	 $sql = mysqli_query($dbhandle,"SELECT email FROM users_tbl WHERE email='".$email."'  LIMIT 1");
	...
	if(confirm('User with that email already
	 exists. try registering new user with new email')){
		window.location = './signup_user.php';
	}
	</script>";
	
		header('location:signup_user.php?duplicate');
	exit();
		...
		$sql = mysqli_query($dbhandle,"INSERT INTO users_tbl(firstname,lastname,phonenumber,email,county,password,idnumber,prof_pic,time_of_signup) VALUES ('".$firstname."','".$lastname."','".$phone."',
		'".$email."','".$county."','".$pass."','". $idno."','$target',now())") or die (mysqli_error($dbhandle));
